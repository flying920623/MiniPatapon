...
public class Level3Stage extends GameStage {
	...
	public void init() {
		...
		MiniPataponManager
				...
				.getBackScreen()
				.navigate(
						new BackgroundStage(MiniPataponManager.getInstance()
								.GetLayeredScreen().getBackScreen(), width,
								height, stretch, ParaBackgroundLevel3.class));

		// new LineImage(this);

		DefaultLogger.getDefaultLogger().logWithSignature(this,
				"x, y :  %f, %f", enemy.x, enemy.y);
	}
			...
			for (TiledObject object : group.objects) {
				if ("standard_bearer".equals(object.name)) {
					...
					hero = SpriterObject.initSpriterObject("blueHero",
							"blueHeroSCML", "move", object.x, -7, 0.6f, 0.6f,
							this, SpriterObject.moveAttack);
				} else if ("toxotae".equals(object.name)) {
					toxotae = SpriterObject.initSpriterObject("toxotae",
							"toxotaeSCML", "move", object.x, 0, 0.5f, 0.5f,
							this, SpriterObject.moveAttack);
				} else if ("bird".equals(object.name)) {
					bird = SpriterObject.initSpriterObject("bird", "birdSCML",
							"move", object.x, object.y / 2, 1, 1, this,
							SpriterObject.throwAttack);
				} else if ("axe".equals(object.name)) {
					axe = SpriterObject.initSpriterObject("axe", "axeSCML",
							"move", object.x, 0, 1, 1, this,
							SpriterObject.throwAttack);
				} else if ("enemy".equals(object.name)) {
					enemy = SpriterObject.initSpriterObject("monster",
							"monsterSCML", "attack", object.x, 0, 2, 2, this,
							SpriterObject.moveAttack);
					enemy.speed = 50;
	...
	public void initSound() {
		...
		// this.thunderSound =
		// ResourceLoader.loadMusic(SoundAssets.thunderSoundStr);

		this.gameCompleteSound = ResourceLoader.getMusic(SoundAssets.missionCompleteSoundStr);
		this.gameFailedSound = ResourceLoader.getMusic(SoundAssets.missionFailedSoundStr);

		this.forwordSound1 = ResourceLoader.getMusic(SoundAssets.forwordSoundStr);
		//
		// // this.forwordSound2 = ResourceLoader
		// // .loadMusic(SoundAssets.forwordSound2Str);
		this.atkSound1 = ResourceLoader.getMusic(SoundAssets.atkSoundStr);
		// this.atkSound2 = ResourceLoader.loadMusic(SoundAssets.atkSound2Str);
		this.defSound1 = ResourceLoader.getMusic(SoundAssets.defSoundStr);
		// this.defSound2 = ResourceLoader.loadMusic(SoundAssets.defSound2Str);
		// this.cheerSound =
		// ResourceLoader.loadMusic(SoundAssets.cheerSoundStr);
		this.failSound = ResourceLoader.getMusic(SoundAssets.failSoundStr);
		//
		...
		//
		// this.deathSound =
		// ResourceLoader.loadMusic(SoundAssets.deathSoundStr);
	}

	public void initMusic() {
		this.tumMusic = ResourceLoader.getMusic(MusicAssets.drumTwoRhythmStr);
		// this.backMusic =
		// ResourceLoader.loadMusic(MusicAssets.level3MusicStr);
	}
	...
	public void OnForwardCommand() {
		...
		if (meetEnemy) {
			new SlideDialog("Kill the Dragon, or you can't move!", hero, this,
					2);
			return;
		}
		...
		if (!meetEnemy) {
			...
			for (int i = 0; i < root.getActors().size(); i++) {
				...
				if (root.getActors().get(i) instanceof SpriterObject) {
					...
					SpriterObject object = (SpriterObject) root.getActors()
							.get(i);
					if (object == enemy)
						continue;
	...
	public void OnFailCommand() {
		...
		if (!meetEnemy) {
			new SlideDialog(
					"Match the rhythm! 'PATA' 'PATA' 'PATA' 'PON' will forword...",
					hero, this, 2);

		} else {
			new SlideDialog(
					"'PON' 'PON' 'PATA' 'PON' will Atack enemy!, Just do it!",
					hero, this, 2);
		}
	...
	public void OnAttackCommand() {
		...
		if (!meetEnemy) {
			...
			standard_bearer.action(MoveAction
					.jump(standard_bearer, height / 10));
	...
	public void killEnemy() {
		...
		Timeline.createSequence().pushPause(0.5f)
				.setCallback(new TweenCallback() {
					...
					public void onEvent(int type, BaseTween<?> source) {
						...
						for (int i = 0; i < root.getActors().size(); i++) {
							if (root.getActors().get(i) instanceof SpriterObject) {
								SpriterObject object = (SpriterObject) root
										.getActors().get(i);
								if (object == enemy)
									continue;
